...
type GturbineFactory func(
	origData []byte,
	nDataShards, nParityShards int,
) (gturbine.Encoder, gturbine.Reconstructor)

func TestGturbineReconstructionCompliance(t *testing.T, f GturbineFactory) {
	t.Helper()
			...
			for _, dataSize := range []int{
				...
				t.Run(fmt.Sprintf("data size = %d", dataSize), func(t *testing.T) {
					...
					for _, idx := range rng.Perm(len(allShards)) {
						...
						require.ErrorIs(t, err, gturbine.ErrIncompleteSet)
					}
